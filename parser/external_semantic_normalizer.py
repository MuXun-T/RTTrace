...
def normalize_btf(trace:ParsedBtfTrace)->tuple[SemanticEvent,...]:
 ...
 for row in trace.records:
  ...
  result.append(SemanticEvent(len(result),row.timestamp,"us","btf",int(row.subject.split("_")[1]) if row.subject.startswith("Core_") else None,None,row.target if row.kind=="T" else None,kind,None,None,None,None,None,None,(("action",row.action),),row.source_record_index))
 return tuple(result)
